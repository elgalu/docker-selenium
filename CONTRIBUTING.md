...
For pull requests or local commits:
    ...
    git checkout -b tmp-3.0.1i && git checkout ./images/grid2_console.png && git checkout ./images/grid3_console.png
    #git add ... git commit ... git push ... open pull request

For repository owners only:

    git commit -m "Add WAIT_TIME_OUT_VIDEO_STOP=20s"
    git tag -d latest && git tag 3.0.1i && git push origin tmp-3.0.1i && git push --tags

-- Wait for Travis to pass OK
-- Make sure changes got merged into master by elgalubot

    git checkout master && git pull && git branch -d tmp-3.0.1i && git push origin --delete tmp-3.0.1i
...
Failed in Travis? retry
    ...
    git tag -d 3.0.1i && git push origin :3.0.1i
    #git add ...
    git commit --amend && git tag 3.0.1i && git push --force origin tmp-3.0.1i && git push --tags
